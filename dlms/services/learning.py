...
def _daily_review_plan(
    ...
    *,
    ...
    """Compose existing recommendations into one deterministic daily plan.

    This function deliberately assigns only category order. Due dates, concept
    ...
    if due_questions:
        ...
        reason = f"{len(due_questions)} source question{'s are' if len(due_questions) != 1 else ' is'} due now."
        if overdue_count:
            reason += f" {overdue_count} {'are' if overdue_count != 1 else 'is'} overdue."
        items.append({
            "id": "native-due",
            "kind": "native_due",
            "priority": 10,
            ...
            "action": {
                ...
                "method": "POST",
                "fields": {"question_count": str(min(20, len(due_questions)))},
            },
        })
    ...
    if selected_concept:
        ...
        accuracy_text = (
            f" Accuracy is {round(float(accuracy))}%." if accuracy is not None else ""
        )
        items.append({
    ...
    if adaptive and not due_questions and selected_concept is None:
        top = adaptive[0]
        evidence = sum(int(candidate.get("evidence") or 0) for candidate in adaptive)
        if evidence == 0:
            title = "Build your learning baseline"
            reason = "You have study material but little recorded history. Start a balanced Adaptive Study session."
        else:
            title = "Study what you need most"
            first_reason = next(
                (
                    str(reason).strip()
                    for reason in top.get("selection_reasons") or []
                    if str(reason).strip()
                ),
                "Existing learning signals identify useful material to practice",
            )
            reason = f"Adaptive Study's highest remaining signal is: {first_reason}."
        items.append({
            "id": "adaptive-study",
            "kind": "adaptive",
            "priority": 40,
    ...
    } if isinstance(installed_content_packs, list) else {}
    ...
    if installed_by_id:
        for event in _deduplicated_learning_answer_events(cur):
            ...
            occurred_at = _parse_learning_datetime(event["occurred_at"])
    ...
    if latest_pack_activity:
        ...
        if days_since <= 30:
            ...
            when = "today" if days_since == 0 else f"{days_since} day{'s' if days_since != 1 else ''} ago"
            items.append({
                "id": f"study-pack-{pack_id}",
                "kind": "study_pack",
                "priority": 50,
                "title": f"Continue {pack_name}",
                "reason": f"Your most recent activity in this Study Pack was {when}.",
                "action": {
    ...
    return {
        ...
        "empty_state": {
            ...
            "detail": "Complete a quiz or begin a Study session to build today's recommendations.",
            "action": {"label": "Browse Quiz Library", "url": "/library"},
        },
        "model": {
            "order": "Due questions, weak/developing concepts, unfinished browser sessions, Adaptive Study, then recent Study Pack activity.",
            "deduplication": "Specific due or concept actions replace broader Adaptive Study suggestions when they would describe the same daily need.",
        },
    }
