...
class ReleaseArtifactVerificationTests(unittest.TestCase):
    ...
    def test_windows_artifact_name_architecture_and_checksum_are_verified(self):
        with tempfile.TemporaryDirectory(prefix="dlms-native-release-") as directory:
            ...
            artifact = root / "DLMS-3.2.0-windows11-x86_64.exe"
            _write_pe(artifact)
            ...
            self.assertIn("Verified DLMS-3.2.0-windows11-x86_64.exe", result.stdout)

    def test_verifier_rejects_obsolete_generic_final_artifact_names(self):
        with tempfile.TemporaryDirectory(prefix="dlms-native-release-") as directory:
            root = Path(directory)
            _write_source_root(root)
            windows = root / "DLMS-3.2.0-windows-x86_64.exe"
            linux = root / "DLMS-3.2.0-linux-x86_64"
            _write_pe(windows)
            _write_elf(linux)
            linux.chmod(0o755)
    ...
    def test_macos_zip_requires_native_bundle_shape_architecture_and_no_runtime_data(self):
        with tempfile.TemporaryDirectory(prefix="dlms-native-release-") as directory:
            root = Path(directory)
            _write_source_root(root)
            artifact = root / "DLMS-3.2.0-macos-arm64.zip"
        ...
        with tempfile.TemporaryDirectory(prefix="dlms-native-release-") as directory:
            ...
            for suffix in (
                ...
                with self.subTest(suffix=suffix):
                    artifact = root / f"DLMS-3.2.0-{suffix}"
                    _write_elf(artifact)
    ...
    def test_linux_native_permission_failure_is_enforced_but_windows_cross_check_is_portable(self):
        with tempfile.TemporaryDirectory(prefix="dlms-native-release-") as directory:
            root = Path(directory)
            artifact = root / "DLMS-3.2.0-fedora44-x86_64"
            _write_elf(artifact)
