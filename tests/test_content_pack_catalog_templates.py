...
class ContentPackCatalogTemplateTests(unittest.TestCase):
    ...
    def test_catalog_empty_state_forms_navigation_flash_and_accessibility(self):
        ...
        self.assertNotIn('id="packSearch"', page)
    ...
    def test_catalog_listing_preserves_counts_badges_actions_and_safe_serialization(self):
        ...
        self.assertIn(f'title="{escape(hostile["name"])}"', page)
        self.assertIn(f'aria-label="More actions and metadata for {escape(hostile["name"])}"', page)
        self.assertIn('class="pack-more" open', page)
    ...
    def test_large_catalog_keeps_identity_actions_and_warning_details(self):
        ...
        self.assertIn('<label for="packSearch">', page)
        self.assertIn('aria-controls="packInventory"', page)
        self.assertIn('id="packFilterEmpty"', page)
