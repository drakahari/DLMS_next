...
For 3.2.1, follow the [final release checklist](releases/3.2.1-RELEASE-CHECKLIST.md).
...
```text
python tools/build_native_release.py --target fedora44-x86_64 --expected-commit FULL_40_CHARACTER_RELEASE_COMMIT --expected-version 3.2.1
```
...
Published 3.2.1 packages and the historical frozen tag are not replaced by this
...
```powershell
Copy-Item dist\DLMS.exe releases\DLMS-3.2.1-windows11-x86_64.exe
python tools\verify_release_artifact.py windows-x86_64 releases\DLMS-3.2.1-windows11-x86_64.exe --smoke
```
...
later contains exactly `DLMS-3.2.1-windows11-x86_64.exe` inside the matching
...
```bash
cp dist/DLMS releases/DLMS-3.2.1-fedora44-x86_64
chmod +x releases/DLMS-3.2.1-fedora44-x86_64
python tools/verify_release_artifact.py linux-x86_64 releases/DLMS-3.2.1-fedora44-x86_64 --smoke

cp dist/DLMS releases/DLMS-3.2.1-ubuntu24.04-x86_64
chmod +x releases/DLMS-3.2.1-ubuntu24.04-x86_64
python tools/verify_release_artifact.py linux-x86_64 releases/DLMS-3.2.1-ubuntu24.04-x86_64 --smoke

cp dist/DLMS releases/DLMS-3.2.1-ubuntu26.04-x86_64
chmod +x releases/DLMS-3.2.1-ubuntu26.04-x86_64
python tools/verify_release_artifact.py linux-x86_64 releases/DLMS-3.2.1-ubuntu26.04-x86_64 --smoke

cp dist/DLMS releases/DLMS-3.2.1-omarchy-quattro-x86_64
chmod +x releases/DLMS-3.2.1-omarchy-quattro-x86_64
python tools/verify_release_artifact.py linux-x86_64 releases/DLMS-3.2.1-omarchy-quattro-x86_64 --smoke
```
...
mkdir -p releases
ditto -c -k --sequesterRsrc --keepParent dist/DLMS.app releases/DLMS-3.2.1-macos-arm64.zip
python tools/verify_release_artifact.py macos-arm64 releases/DLMS-3.2.1-macos-arm64.zip --smoke
```
...
`DLMS-3.2.1-macos-x86_64.zip` after building with native `x86_64` macOS
...
python tools/package_release.py \
  ...
  --artifact releases/DLMS-3.2.1-fedora44-x86_64 \
  --output-dir release-packages \
  --smoke

python tools/package_release.py \
  --target ubuntu24.04-x86_64 \
  --artifact releases/DLMS-3.2.1-ubuntu24.04-x86_64 \
  --output-dir release-packages \
  --smoke

python tools/package_release.py \
  --target ubuntu26.04-x86_64 \
  --artifact releases/DLMS-3.2.1-ubuntu26.04-x86_64 \
  --output-dir release-packages \
  --smoke

python tools/package_release.py \
  --target omarchy-quattro-x86_64 \
  --artifact releases/DLMS-3.2.1-omarchy-quattro-x86_64 \
  --output-dir release-packages \
  --smoke
```
...
python tools\package_release.py `
  ...
  --artifact releases\DLMS-3.2.1-windows11-x86_64.exe `
  --output-dir release-packages `
  --smoke
```
...
python tools/package_release.py \
  --target macos-arm64 \
  --artifact releases/DLMS-3.2.1-macos-arm64.zip \
  --output-dir release-packages \
  --smoke
```
...
```text
DLMS-3.2.1-fedora44-x86_64.tar.gz
└── DLMS-3.2.1-fedora44-x86_64/
    ├── DLMS-3.2.1-fedora44-x86_64
    ├── README.txt
    └── sample_quiz.txt

DLMS-3.2.1-ubuntu24.04-x86_64.tar.gz
└── DLMS-3.2.1-ubuntu24.04-x86_64/
    ├── DLMS-3.2.1-ubuntu24.04-x86_64
    ├── README.txt
    └── sample_quiz.txt

DLMS-3.2.1-ubuntu26.04-x86_64.tar.gz
└── DLMS-3.2.1-ubuntu26.04-x86_64/
    ├── DLMS-3.2.1-ubuntu26.04-x86_64
    ├── README.txt
    └── sample_quiz.txt

DLMS-3.2.1-windows11-x86_64.zip
└── DLMS-3.2.1-windows11-x86_64/
    ├── DLMS-3.2.1-windows11-x86_64.exe
    ├── README.txt
    └── sample_quiz.txt

DLMS-3.2.1-macos-arm64.zip
├── DLMS.app/
├── README.txt
└── sample_quiz.txt

DLMS-3.2.1-omarchy-quattro-x86_64.tar.gz
└── DLMS-3.2.1-omarchy-quattro-x86_64/
    ├── DLMS-3.2.1-omarchy-quattro-x86_64
    ├── README.txt
    └── sample_quiz.txt
```
...
```bash
python tools/verify_release_package.py "$PACKAGE_DIR/DLMS-3.2.1-fedora44-x86_64.tar.gz" --smoke
python tools/verify_release_package.py "$PACKAGE_DIR/DLMS-3.2.1-ubuntu24.04-x86_64.tar.gz" --smoke
python tools/verify_release_package.py "$PACKAGE_DIR/DLMS-3.2.1-ubuntu26.04-x86_64.tar.gz" --smoke
python tools/verify_release_package.py "$PACKAGE_DIR/DLMS-3.2.1-omarchy-quattro-x86_64.tar.gz" --smoke
```
...
```powershell
python tools\verify_release_package.py "$env:PACKAGE_DIR\DLMS-3.2.1-windows11-x86_64.zip" --smoke
```

The Windows flow uses PowerShell `Expand-Archive`, requires the exact versioned
wrapper and stable-named `DLMS-3.2.1-windows11-x86_64.exe`, requires the two
...
```bash
python tools/verify_release_package.py "$PACKAGE_DIR/DLMS-3.2.1-macos-arm64.zip" --smoke
```
...
```bash
PACKAGE_DIR="$PWD/build/release-packages/3.2.1"
python tools/generate_sha256sums.py --output "$PACKAGE_DIR/SHA256SUMS.txt" \
  "$PACKAGE_DIR/DLMS-3.2.1-fedora44-x86_64.tar.gz" \
  "$PACKAGE_DIR/DLMS-3.2.1-ubuntu24.04-x86_64.tar.gz" \
  "$PACKAGE_DIR/DLMS-3.2.1-ubuntu26.04-x86_64.tar.gz" \
  "$PACKAGE_DIR/DLMS-3.2.1-windows11-x86_64.zip" \
  "$PACKAGE_DIR/DLMS-3.2.1-macos-arm64.zip" \
  "$PACKAGE_DIR/DLMS-3.2.1-omarchy-quattro-x86_64.tar.gz"
```
...
1. `DLMS-3.2.1-fedora44-x86_64.tar.gz`
2. `DLMS-3.2.1-ubuntu24.04-x86_64.tar.gz`
3. `DLMS-3.2.1-ubuntu26.04-x86_64.tar.gz`
4. `DLMS-3.2.1-windows11-x86_64.zip`
5. `DLMS-3.2.1-macos-arm64.zip`
6. `DLMS-3.2.1-omarchy-quattro-x86_64.tar.gz`
7. `SHA256SUMS.txt`

GitHub automatically supplies repository source ZIP and tarball links. Do not
create or upload `DLMS-3.2.1-source.zip` or another manual source archive.
...
```text
python tools/accept_downloaded_release.py --repo drakahari/DLMS_next --tag v3.2.1 --asset DLMS-3.2.1-macos-arm64.zip
python tools/accept_downloaded_release.py --repo drakahari/DLMS_next --tag v3.2.1 --asset DLMS-3.2.1-windows11-x86_64.zip
```
