...
| State-dependent behavior | Hidden folders/quizzes are separated from normal display; search and scope can expose matching hidden content. Empty folders and empty searches have specific states. Uncategorized Generated Practice appears in a derived automatic group; an explicit custom-folder assignment overrides that presentation. |
| Integrity | Folder and order changes affect organization, not quiz content, identity, or history. Generated Practice source summaries resolve live source quizzes from question lineage without changing stored quiz/folder data or guessing from text. |
...
| Inventory field | Verified behavior |
| --- | --- |
| Screen / reach | Dashboard Today’s Review or Review Schedule → Review Due Questions. |
...
| DAF-07 | The direct Short Quiz Builder choice contract is verified at both browser and server boundaries. | The UI starts with four rows, supports 1–26 nonblank choices, requires at least one nonblank and one marked correct choice, ignores blank unused rows, and derives single-answer versus multi-select from the number marked correct. Both the browser and POST route enforce the A–Z / 26-choice boundary. | Document the supported 1–26 choice contract. Crafted submissions outside A–Z or above 26 choices are rejected. |
...
| DAF-10 | Generated sessions are persistent Library records with a completion presentation, but no automatic deletion or expiration. | Completed transient reviews leave the active Generated Practice group for a collapsed Completed Generated Practice group; the Smart View finds both, while custom folders stay assigned. Source evidence, History, and schedules remain. | Explain active versus completed Library status and its separation from browser-local Resume; do not imply cleanup of stored quiz data. |
