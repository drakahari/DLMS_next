...
| **Multi-select question** | A choice question with more than one correct answer. | Builders and quiz runtime. | Import and Review & Repair screens may call the same behavior “multiple-answer.” | Prefer *multi-select* in general explanation; quote *multiple-answer* when naming the visible mode. Contrast with a single-answer choice question. |
...
| **Smart PDF** | DLMS’s structured selectable-text PDF analysis, classification, bank creation, and carefully bounded OCR assistance. | Root product description and this manual; the current in-application screen and Help topic are named **PDF & Image Import**. | Smart PDF is not a separate button or generic visual understanding of arbitrary textbooks. | Use **PDF & Image Import** for the screen users must find. Use *Smart PDF* only when explaining the structured analysis within that workflow, then lead into Review & Repair. |
